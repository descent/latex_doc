...
We load the address of name[] into the EAX register.0x8000149 : pushl %eax 
...
Copy 0xb (1 1 decimal) onto the stack. This is the index into the syscall table. 11 is execve. 
...
As we can see there is not much to the execve() system call. All we need to do is: a. Have the null terminated string "/bin/sh" somewhere in memory. 
...
a. Have the null terminated string "/bin/sh" somewhere in memory. b. Have the address of the string "/bin/sh" somewhere in memory followed by a null long word. 
...
__asm__("
        ...
        movb   $0x0,0x7(%esi)           # 4 bytes        movl   $0x0,0xc(%esi)           # 7 bytes
...
__asm__("
        ...
        movl   %ebx,%eax                # 2 bytes        inc    %eax                     # 1 bytes
...
large_string onto buffer without doing any bounds checking, and wil l overflow the return address, overwriting it 
