...
\begin{lstlisting}[caption=xx, basicstyle=\footnotesize, breaklines=true]
int function(void) 
{
  int i;
  for (i = 0; i < 10; i++)
...
\begin{lstlisting}[caption=xx, basicstyle=\footnotesize]
int function(void) {
...
\begin{lstlisting}[caption=yy, basicstyle=\footnotesize, breaklines=true]
int function(void) 
{
  static int i, state = 0;
  switch (state) 
...
\section{References}
    ...
    PuTTY is a Win32 Telnet and SSH client. The SSH protocol code contains real-life use of this coroutine trick. As far as I know, this is the worst piece of C hackery ever seen in serious production code. tion using a goto statement. So if we use a state variable, we could do this:
