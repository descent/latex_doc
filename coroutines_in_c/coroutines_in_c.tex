...
如何實作這個呢? 好的, 我們可以用一個 goto 語句將控制跳轉到這個函數中的任何一點。所以如果我們有一個狀態變數, 我們可以這樣做:
...
這個方法可以奏效。在一些我們可能需要恢復控制的地方, 我們擁有一組標籤: 一個在開始位置, 其他的緊跟著每個返回語句。我們有一個保留在函數呼叫之間的狀態變數, 它指明了下一步我們需要恢復控制那個標籤。在任何返回之前, 我們會更新狀態變數到正確的標籤位;
...
acrReturn 中的靜態局部變數; 絕不要在一個顯式 switch 語句中設置一個 crReturn); 但那些並不會限制我們太多。
